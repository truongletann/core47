        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-[rgb(var(--border))] text-xs uppercase text-[rgb(var(--muted))]">
              <th className="px-4 py-2">Cảnh</th>
          ...
          <tbody>
            {loading ? (
              ...
              SCENES.map((s) => {
                ...
                return (
                  <tr key={s.key} className="border-b border-[rgb(var(--border))] last:border-0">
                    <td className="px-4 py-2">{s.name}</td>
                    <td className="px-4 py-2 text-xs">
                      {bg ? `${bg.mediaType} · ${bg.source}` : <span className="text-[rgb(var(--muted))]">Mặc định (canvas)</span>}
                    </td>
